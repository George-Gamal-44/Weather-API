...
function displayWeather(data) {
    var box = `
    <!-- card 1 -->
    <div class="col-12 col col-md-6 col-lg-4"><div class="inner color3 rounded-4 text-white px-0 overflow-hidden">
        <div class="date color2 fw-medium d-flex justify-content-between align-items-center px-4 border-bottom border-info">
<p class="my-2">${getDateName(data.forecast.forecastday[0].date)}</p>
<p class="my-2">${getDateAndMonth(data.forecast.forecastday[0].date)}</p>
        </div>
        <div class="info px-4">
...
<div class="temp d-flex justify-content-between align-items-center mt-5 pe-4">
    ...
    <img src="https:${data.current.condition.icon}" alt="icon" class="w-25" />
    </div>
    <p class="statu mt-2 fs-6 text-info">${data.current.condition.text}</p>
        </div>
        <div class="wind px-4 mt-5 mb-3">
    <span class="me-4"><i class="fa-solid fa-umbrella me-2 text-info"></i>${data.current.feelslike_c}%</span>
    <span class="me-4"><i class="fa-solid fa-wind me-2 text-info"></i>${data.current.wind_kph}km/h</span>
    <span><i class="fa-solid fa-compass me-2 text-info"></i>${data.current.wind_dir}</span>
        </div>
</div>
    </div>
    <!-- card 2 -->
    ...
    <div class="inner color4 rounded-4 text-white px-0 overflow-hidden h-100">
        <div class="date color5 fw-medium d-flex justify-content-between px-4 border-bottom border-info">
        <p class="my-2">${getDateName(data.forecast.forecastday[1].date)}</p>
        <p class="my-2">${getDateAndMonth(data.forecast.forecastday[1].date)}</p>
        </div>
...
<img src="https:${data.forecast.forecastday[1].day.condition.icon}" alt="icon" class="w-25 mt-5" />
    ...
    <div class="inner color3 rounded-4 text-white px-0 overflow-hidden h-100">
        <div class="date color2 fw-medium d-flex justify-content-between px-4 border-bottom border-info">
        <p class="my-2">${getDateName(data.forecast.forecastday[2].date)}</p>
        <p class="my-2">${getDateAndMonth(data.forecast.forecastday[2].date)}</p>
        </div>
        <div class="info px-4 text-center">
